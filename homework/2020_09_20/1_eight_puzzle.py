...
# ref: https://gist.github.com/flatline/838202/ca0d35b0ce7e5d9ec86b77b0490baba4cda87980
...
def click_move(row, col):
    global p, window
    print(row, col)
    zero_p = p.find(0)
    zero_row = zero_p[0]
    zero_col = zero_p[1]
    if zero_row == row and abs(zero_col - col) == 1:
        p.swap(((row,col)), zero_p)
    elif zero_col == col and abs(zero_row - row) == 1:
        p.swap(((row,col)), zero_p)
    else:
        print("can't move")
    show_eight_puzzle()
...
def show_eight_puzzle():
    ...
    for i in range(3):
        for j in range(3):
            ...
            label.bind("<Button-1>",lambda e,row=i, col=j:click_move(row, col))
...
def click_solve():
    ...
    for i in path:
        time.sleep(0.5)
        print(i)
        p.adj_matrix = i.adj_matrix
        show_eight_puzzle()
        window.update()
    print("Solved", len(path), "steps") 
...
def main():
    ...
    p = EightPuzzle()
    click_shuffle()
    btnRandom = tk.Button(text="Random")
    btnRandom.grid(row=3, column=0)
    btnRandom.bind('<Button-1>', lambda e, window=window:click_shuffle())
    btnSolve = tk.Button(text="Solve")
    btnSolve.grid(row=3, column=1)
    btnSolve.bind('<Button-1>', lambda e, window=window:click_solve())
    window.mainloop()
